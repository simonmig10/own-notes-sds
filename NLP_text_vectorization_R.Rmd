...
```{r}
text_tidy %<>%
  unnest_tokens(word, word, token = 'ngrams', n = 2, n_min = 1) %>% ## We have both bio grams and single words, we see that biograms are not that important tho
...
library(stopwords)
recipe_base %>% 
  step_tf(text) %>%  #We can add steps to the recipe 
  prep() %>% 
  juice() %>% 
  head(100)
```
...
recipe_pca %>%
  ...
  labs(y = NULL)
```
We can see negative or positive words in each component. 
