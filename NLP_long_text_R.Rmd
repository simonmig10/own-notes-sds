...
el_words <- text_tidy %>%
  ...
  rename(from = item1, to = item2, weight = n) #Which words apears in the same line
```
...
text_tidy_ngrams <- text %>% 
  unnest_tokens(bigram, text, token = 'ngrams', n = 2) %>% #ngram --> hvor mange ord vi sætter sammen --> n=2 så det et biogram. 
  na.omit()
```
...
```
We can see the ending word is the starting word in the next one. 
